...
练了3个网络，都不收敛，我们的12导联心电数据只有177例，其中每一个数据是12（通道）*400（帧）的心电信号，同时还有着数据不均衡的问题（如果只是随机采样，全部预测正例acc都有0.77），我把详细的调参经过和每一轮的结果都留档了；于是我又采用了3种机器学习方法，通过网格优化保证参数不影响结果，但是仍然阳性分不出来  
我们5分钟的300000帧数据叠加到400帧，这个数据量不仅跑不起TCN，连XGBOOST效果都不会太好  
现在最需要改进的还是数据预处理部分，叠加后至少保留400*100的数据，也就是每个csv里面要有12列，40000行  
